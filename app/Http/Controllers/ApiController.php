...
class ApiController extends Controller
{
        ...
        try{  
            ...
            if(empty($error)) {
                ...
                if($status != 1) {
                    ...
                    if($userid > 0) { 
                        $brands = Brands::where('status', 'ACTIVE')->orderby('position', 'asc')->get();
                           // ->skip($page_no)->take($limit)
                        if($brands->isNotEmpty()) {
                            return response()->json(['status' => 1, 'data' => $brands, 'message' => 'Brands List']);
                        }   else {
                            return response()->json(['status' => 0, 'data' => null, 'message' => 'No Brands List']);
                        }
                    }   else {
    ...
    */
    ...
    /* Guest User Login */
    public function postGuestUserLogin(Request $request) {  
        $inputJSON = file_get_contents('php://input');

        $input = json_decode($inputJSON, TRUE);

        $username = 'guest@emruv.com'; //$input['email'];

        $password = 'emruv@123'; //$input['password'];
        ...
        if (Auth::attempt(['email' => $username, 'password' => $password])) {
            ...
            $user = User::where('email', $username)->where('user_type', 'GUESTUSER')->first();

            $expiry = $user->api_token_expiry;
            ...
            return response()->json(['status' => 1, 'data' => $user, 'message' => 'Successfully Login']);

        } else if (Auth::attempt(['mobile' => $username, 'password' => $password])) {

            $user = User::where('mobile', $username)->where('user_type', 'GUESTUSER')->first();
            ...
            return response()->json(['status' => 0, 'data' => '', 'message' => 'Invalid credential']);
            ...
            if(empty($error)) {
                ...
                if(!empty($mobileEx)) {  // registered user
                    if($mobileEx->status != 'ACTIVE') {
                       ...
                       // CommonController::auth($mobileEx->id);
                        ...
                        if($is_register_complete == 0) {
                            return response()->json(['status' => 1, 'message' => 'Login Successful. Update Profile', 'data' => $user]);
                        }   else {
                    ...
                    if (!empty($user)) {
                        ...
                        return response()->json(['status' => 1, 'message' => 'Successfully Registered.', 'data' => $user]);
    ...
    public function postUserAddress(Request $request)   {
        ...
        try {
            ...
            if(empty($error)) {
                ...
                $is_default = ((isset($input) && isset($input['is_default']))) ? $input['is_default'] : 0;  
...
/*
    ...
    */
    ...
    public function postInsertGeneralBooking(Request $request) {
        //try {   
            ...
            if(empty($error)) {
                ...
                $brand_id = ((isset($input) && isset($input['brand_id']))) ? $input['brand_id'] : 0; 
    ...
    public function calculateFareBreakup_old($userid, $service_user_id, $booking_true=false, $booking_id=0) {
        if(!$booking_true) {
    ...
    public function calculateFareBreakup_new($userid, $service_user_id, $booking_true=false, $booking_id=0) {
        if(!$booking_true) {
            $cartsubservices = UserCartSubServices::where('user_id', $userid)
            ->select(DB::RAW('GROUP_CONCAT(sub_category_id) as usersubcategories'), DB::RAW('GROUP_CONCAT(sub_service_id) as usersubservices'), DB::RAW('GROUP_CONCAT(service_id) as userservices'))->first(); 
        } else {
            $cartsubservices = BookingSubServices::where('booking_id', $booking_id)
            ->select(DB::RAW('GROUP_CONCAT(sub_category_id) as usersubcategories'), DB::RAW('GROUP_CONCAT(sub_service_id) as usersubservices'), DB::RAW('GROUP_CONCAT(service_id) as userservices'))->first();
        }  
        $servicer_services = []; $cart_details = [];
        if(!empty($cartsubservices)) {
            $usersubcategories = $cartsubservices->usersubcategories;
            if(!empty($usersubcategories)) {
                $user_subcategories = explode(',', $usersubcategories);
                $user_subcategories = array_unique($user_subcategories);
 
                $servicer_services_arr = DB::table('em_service_provider')
                    ->where('user_id', $service_user_id)->value('sub_category_ids');

                $servicer_sub_services_arr = [];
                if(!empty($servicer_services_arr)) {
                    $servicer_services_arr = explode(',', $servicer_services_arr);
                    $servicer_services_arr = array_unique($servicer_services_arr);
                    if(count($servicer_services_arr)>0) {
                        $servicer_sub_services = DB::table('em_sub_service')
                            ->leftjoin('em_sub_cat_services', 'em_sub_cat_services.id', 'em_sub_service.service_id')
                            ->whereIn('em_sub_cat_services.id', $servicer_services_arr)
                            ->select('id')->get();
                        if($servicer_sub_services->isNotEmpty()) {
                            foreach($servicer_sub_services as $ssk => $ssv) {
                                $servicer_sub_services_arr[] = $ssv->id;
                ...
                if(count($servicer_sub_services_arr)>0) {
                    $servicer_sub_services_arr = array_unique($servicer_sub_services_arr);
                    $servicer_service_details = SubServices::whereIN('id', $servicer_sub_services_arr)->get();
            ...
            if(empty($error)) {
                ...
                if($status != 1) {
                    ...
                    if($userid > 0) {  
                        ...
                        $maxcount_zones = DB::table('em_admin_settings')->where('id', 1)->value('max_count_zone');

                        if($zones->isNotEmpty()) {                         
                            return response()->json([ 'status' => 1, 'data' => $zones, 'message' => 'Zones List', 'maxcount_zones' => $maxcount_zones]);
                        }   else {
                            return response()->json([ 'status' => 0, 'data' => [], 'message' => 'No Zones', 'maxcount_zones' => $maxcount_zones]);
                        } 
                    }   else {
                        return response()->json([ 'status' => 0, 'data' => [], 'message' => 'Invalid Inputs']);
                    }
                    ...
                    if($mobileEx->status != 'ACTIVE') {
                       ...
                       // CommonController::auth($mobileEx->id);
                        ...
                        if($is_register_complete == 0) {
                            ...
                            return response()->json(['status' => $sendstatus, 'message' => 'Login Successful. Update Profile', 'data' => $user]);
                        }   else {
                            return response()->json(['status' => 1, 'message' => 'Login Successful.', 'data' => $user]);
                        }
                    }
            ...
            if(empty($error)) {
                ...
                if($status != 1) {
                    ...
                    if($userid > 0) { 
                        ...
                        $exuser = ServiceProvider::where('user_id', $userid)->first();
                        if(!empty($exuser)) {
                            if(!empty($exuser->noc_front) && !empty($exuser->driving_license_back) && !empty($exuser->driving_license_front) && !empty($exuser->id_proof_front) && !empty($exuser->id_proof_back)) {
    ...
    public function postServicerUpdateStep4_new() {
        ...
        try {   
            ...
            $requiredParams = ['user_id', 'api_token', 'category_ids', 'sub_category_ids'];
            ...
            if(empty($error)) {
                ...
                if($status != 1) {
                    ...
                    if($userid > 0) { 
                        ...
                        $category_ids = ((isset($input) && isset($input['category_ids']))) ? $input['category_ids'] : "";   
    ...
    /* Post Servicer Update Latitude and Longitude Details */
    public function postServicerUpdateLatLong(Request $request) {
        ...
        try {   
            ...
            $requiredParams = ['user_id', 'api_token', 'latitude', 'longitude'];
            ...
            if(empty($error)) {
                ...
                $userid = ((isset($input) && isset($input['user_id']))) ? $input['user_id'] : 0;  
                $latitude = ((isset($input) && isset($input['latitude']))) ? $input['latitude'] : '';  
                $longitude = ((isset($input) && isset($input['longitude']))) ? $input['longitude'] : '';  
        ...
        try {   
            ...
            if(empty($error)) {
                ...
                if($status != 1) {
                    ...
                    $user = CommonController::getUserDetails($userid);
                    if(!empty($user)) {
                        return response()->json(['status' => 1, 'message' => 'Profile Image Uploaded Successfully', 'data' => $user]);
                    }   else {
                        return response()->json([ 'status' => 0, 'data' => [], 'message' => 'Invalid User']);
                    }
            ...
            if(empty($error)) {
                ...
                if($status != 1) {
                    ...
                    if($userid > 0) { 
                        ...
                        $maxcount_subcategory = DB::table('em_admin_settings')->where('id', 1)->value('max_count_subcategory');
                        ...
                        if(!empty($sub_category)){
                            return response()->json(['status' => 1, 'message' => 'Sub Category List', 'details'=>$sub_category, 'maxcount_subcategory'=>$maxcount_subcategory]);
                        }   else {
                            return response()->json(['status' => 0, 'message' => 'No Sub Category List', 'maxcount_subcategory'=>$maxcount_subcategory]);
                        }
                    }   else {
                        return response()->json([ 'status' => 0, 'data' => [], 'message' => 'Invalid Inputs']);
                    }
                    ...
                    if($userid > 0) {
                        ...
                        if($service_provider_id>0) {
                            ...
                            $servicer = DB::select('SELECT  em_servicer_leave_slots.*,
                                    ...
                                    and servicer_id = "'.$service_provider_id.'" 
                                    GROUP BY em_servicer_leave_slots.id');
                           ...
                           // echo "<pre>"; print_r($slots); 
                            if($device_type == 'ANDROID') {
                                if(empty(trim($leave_date))) {
                                    $start_date = date("Y-m-d", strtotime($year.'-'.$month.'-'.date('d')));  
                                  //  $from_date = date("Y-m-d", strtotime($year.'-'.$month.'-01'));  
                                      $from_date = date("Y-m-d", strtotime($year.'-'.$month.'-'.date('d')));  
